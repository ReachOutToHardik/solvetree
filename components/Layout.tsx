...
import { Network, MessageSquare, Map, Menu, X, Twitter, Linkedin, Github } from 'lucide-react';
            ...
            <nav className="hidden md:flex space-x-8">
              {navItems.map((item) => (
                <a
                  ...
                  className={`flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-md transition-all duration-200 cursor-pointer ${
                    isActive(item.path)
            ...
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              {navItems.map((item) => (
                <a
                  ...
                  href={`#${item.path}`}
                  onClick={handleNav(item.path)}
                  className={`flex items-center gap-3 px-3 py-4 text-base font-medium rounded-md cursor-pointer ${
                     isActive(item.path)
                      ? 'text-brand-600 bg-brand-50'
                      : 'text-slate-600 hover:text-brand-600 hover:bg-slate-50'
                  }`}
